    ...
    def bug_algorithm_callback(self):
        ...
        if self.state == 'directions':
            ...
            if angle_diff > math.pi:
                ...
                angle_diff += 3 * math.pi
            self.cmd_vel_msg.angular.z = 0.25 * angle_diff
            self.cmd_vel_msg.linear.x = 0.0
            ...
            if not self.direction_found:
                return
            if self.fl_sensor_value < 0.70 or self.fr_sensor_value < 0.70:
            ...
            if self.fl_sensor_value > 0.70 and self.fr_sensor_value > 0.70:
                self.state = 'move_to_goal'
                ...
                self.cmd_vel_msg.angular.z = 0.1
                self.cmd_vel_msg.linear.x = 0.0
                if self.is_on_line() and self.calculate_distance(self.current_x, self.current_y, self.goal_x, self.goal_y) < 0.1:
                    self.state = 'move_to_goal'
                    self.cmd_vel_msg.angular.z = 0.0
                    self.cmd_vel_msg.linear.x = 0.0
        self.cmd_pub.publish(self.cmd_vel_msg)
